    ...
    entries.forEach(function (entry) {
      if (entry.intersectionRatio > ratioA) {
        if (window.screen.width > 992) {
          setTimeout(() => {
            entry.target.style.background = `#e1dbd6 url(images/realisation-intro.webp) no-repeat bottom right fixed`;
          }, 500);
        } else {
          setTimeout(() => {
            entry.target.style.background = `#e1dbd6 url(images/realisation-intro-mini.webp) no-repeat bottom right fixed`;
          }, 500);
        }
...
function creationsIntro() {
  ...
  const handleIntersect = function (entries, observer) {
    entries.forEach(function (entry) {
      if (entry.intersectionRatio > ratioA) {
        setTimeout(() => {
          entry.target.style.background = `#e1dbd6 url(images/divers.webp) no-repeat top fixed`;
        }, 500);
